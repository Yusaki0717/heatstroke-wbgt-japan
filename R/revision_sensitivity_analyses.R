...
# Uses af_df produced by spatial_maps_and_af.R (columns: pref_code, period,
# total_ead, an_heat, ...). We restrict to common_pref for ALL periods.

if (!exists("af_df")) stop("af_df not found. Run spatial_maps_and_af.R first.")
...
# Prediction basis: 22-35 degC grid (100 points), Boundary.knots = pooled range.
# This is the ORIGINAL extraction path for the severe-case row (original
# supplementary_analysis.R, block G) and reproduces Table 4 exactly
# (8.14/4.97/5.04). The full-range grid used for Table 2 shifts the pooled
# severe RR by ~1.5%; exact-point evaluation shifts it by ~3%.
wbgt_range_loc <- range(wbgt_all)
wbgt_pred      <- seq(22, 35, length.out = 100)
basis_var      <- ns(wbgt_pred, knots = knots_var, Boundary.knots = wbgt_range_loc)
...
cat("\nPooled severe-case RR at P95 (should reproduce Table 4 exactly:\n")
cat("8.14 (6.96-9.51) / 4.97 (3.88-6.37) / 5.04 (3.84-6.62)):\n")
...
if (exists("af_severe_df")) {
  ...
  severe_an_tbl <- NULL
  cat("\n(af_severe_df not found; run spatial_maps_and_af.R severe block for AN.)\n")
}
